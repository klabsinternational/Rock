    ...
    internal class AuthorizationProvider : OpenIdConnectServerProvider
    {
        ...
        public override async Task ValidateAuthorizationRequest( ValidateAuthorizationRequestContext context )
        {
                 ...
                 context.Request.HasResponseType( OpenIdConnectConstants.ResponseTypes.Token ) )
            {
                context.Reject(
                    ...
                    description: "Only response_type=code and response_type=id_token or response_type=code id_token are supported by this authorization server." );
            ...
            if ( !string.IsNullOrEmpty( context.RedirectUri ) &&
                !string.Equals( context.RedirectUri, authClient.RedirectUrl, StringComparison.OrdinalIgnoreCase ) )
            {
                context.Reject(
                    error: OpenIdConnectConstants.Errors.InvalidClient,
            ...
            context.Validate( authClient.RedirectUrl );
        }
        ...
        public override async Task ValidateLogoutRequest( ValidateLogoutRequestContext context )
        {
            ...
            if ( !context.PostLogoutRedirectUri.IsNullOrWhiteSpace() )
            {
                var rockContext = new RockContext();
                var authClientService = new AuthClientService( rockContext );
                var authClient = await authClientService.GetByLogoutRedirectUrl( context.PostLogoutRedirectUri );
